...
import json, copy, os, io, tempfile
import numpy as np
import pandas as pd
...
def run_simulation_advanced_cached(schedule_json, iterations, start_date_iso, use_super):
    ...
    p90 = float(np.percentile(samples, 90)) if len(samples) else 0.0
...
class State:
    ...
    def run_simulation(self, iterations=2000):
        try:
            ...
            # Defensive: record empty results and surface error via return
            self.sim_results = []
            self.crit_index = {}
...
class StateManager:
    ...
    def apply_delta(self, deltas):
        for t in self.B.schedule:
            if t["id"] in deltas:
                new_dur = float(t.get("duration", 0)) + float(deltas[t["id"]])
                # Clamp to non-negative to avoid negative-duration cycles
                t["duration"] = max(0.0, new_dur)
...
def main():
    ...
    sel_pattern = st.sidebar.selectbox("Select Project Portfolio", list(patterns.values()))
    ...
    # Feature Upgrade: Allow user to toggle super nodes
    use_super = st.sidebar.checkbox("Enable Super Source/Sink Nodes", value=True, help="Standardizes disjointed schedules by forcing a single start/end point.")

    # State tracking: use a compact, stable state key
    # Use a lightweight hash of the baseline schedule to avoid heavy comparisons
    base_sch_hash = hashlib.sha256(json.dumps(base_sch, sort_keys=True).encode('utf-8')).hexdigest()
    state_key = (sel_key, project_start.isoformat(), int(iters), bool(use_super), base_sch_hash)

    if "state_manager_key" not in st.session_state or st.session_state.state_manager_key != state_key:
        st.session_state.state_manager = StateManager(base_sch, start_date=project_start, use_super=use_super)
        st.session_state.state_manager_key = state_key
    ...
    # Ensure crit_index exists before sorting
    ...
    p90_ref = float(np.percentile(ref_samples, 90)) if len(ref_samples) else 0.0
    p90_curr = float(np.percentile(curr_samples, 90)) if len(curr_samples) else p90_ref
    exposure_delta = (p90_curr - p90_ref) * ld_rate

    mean_curr = float(np.mean(curr_samples)) if len(curr_samples) else 0.0
    mean_ref = float(np.mean(ref_samples)) if len(ref_samples) else 0.0
    ...
    with left:
        ...
        # quantile_graph can be expensive; keep parameters explicit
        ...
        # visualizetopology historically returned a file path; read and cleanup immediately to avoid disk growth
        ...
        try:
            ...
            # Provide a download button using in-memory bytes
            st.download_button("📥 Export Dynamic Topology (HTML)", html_content.encode('utf-8'), "Network_Report.html", "text/html")
        except Exception as e:
            st.error(f"Unable to render topology: {e}")
        finally:
            # Attempt to remove the temporary file if it exists and is not a shared asset
            try:
    ...
    with right:
        ...
        fig = go.Figure()
        base_arr = np.array(ref_samples) if len(ref_samples) else np.array([])
        delayed_arr = np.array(curr_samples) if len(curr_samples) else np.array([])

        # Defensive: ensure arrays are non-empty before min/max
        if base_arr.size == 0 and delayed_arr.size == 0:
            base_arr = np.array([0.0])
            delayed_arr = np.array([0.0])
        ...
        bin_count = 40
        bin_size = (xmax - xmin) / bin_count if bin_count > 0 else 1.0
    ...
    with rl_left:
        ...
        # Polish: Colored bars based on correlation direction
    ...
    with memo_right:
        st.write("") # Spacer
        st.write("") # Spacer
        generate_memo = st.button("🤖 Generate Flash Report", use_container_width=True)
        
    if generate_memo:
        if float(delay) == 0.0:
            st.info("No scenario change detected — memo not generated.")
        else:
