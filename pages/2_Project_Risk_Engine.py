...
# --- NEW: CSV Parsing Helper Function ---
def parse_csv_to_schedule(uploaded_file):
    try:
        df = pd.read_csv(uploaded_file)
        
        # Ensure we don't crash if they use slightly different headers (case-insensitive)
        df.columns = [str(c).strip().lower() for c in df.columns]
        
        # Fallback to column indices if standard names aren't found
        id_col = 'id' if 'id' in df.columns else df.columns[0]
        dur_col = 'duration' if 'duration' in df.columns else df.columns[1]
        pred_col = 'predecessors' if 'predecessors' in df.columns else (df.columns[2] if len(df.columns) > 2 else None)
        ...
        for _, row in df.iterrows():
            ...
            # Safely parse comma-separated predecessors
            preds = []
            if pred_col and pd.notna(row[pred_col]):
                raw_preds = str(row[pred_col]).split(',')
                preds = [p.strip() for p in raw_preds if p.strip()]
                
            custom_schedule.append({"id": task_id, "duration": duration, "predecessors": preds})
...
def main():
    ...
    # --- DATA INGESTION ---
    uploaded_csv = st.sidebar.file_uploader("Upload Custom Schedule (CSV)", type=["csv"])
    
    if uploaded_csv is not None:
        base_sch = parse_csv_to_schedule(uploaded_csv)
        sel_pattern = "Custom CSV Upload"
        if not base_sch:
            st.sidebar.warning("Failed to parse CSV. Reverting to healthy pattern.")
            base_sch = schedulesdb["schedulehealthy"]
            sel_pattern = "✅ Healthy Linear"
    else:
        patterns = {
        ...
        sel_pattern = st.sidebar.selectbox("Test Patterns", list(patterns.values()))
    ...
    # Initialize StateManager - Resets automatically if a new CSV is uploaded or pattern changes
